...
// UDPRelayServerOption carries the internal server settings for SIP003U UDP relay.
type UDPRelayServerOption struct {
	ListenAddr  string
	TargetAddr  string
	Certificate string
	PrivateKey  string
...
func NewUDPRelayServer(option UDPRelayServerOption) (*UDPRelayServer, error) {
	timeout := udpRelayTimeout(option.Timeout)
	certLoader, err := ca.NewTLSKeyPairLoader(option.Certificate, option.PrivateKey)
	if err != nil {
		return nil, err
	}
	packetConn, err := net.ListenPacket("udp", option.ListenAddr)
	if err != nil {
		return nil, err
	}
	tlsConfig := &tls.Config{
	...
	listener, err := quic.Listen(packetConn, tlsConfig, udpRelayQUICConfig(timeout))
	if err != nil {
		_ = packetConn.Close()
		return nil, err
	}
...
func (s *UDPRelayServer) Serve(ctx context.Context, targetAddr string) error {
	for {
		...
		s.conns[conn] = struct{}{}
		s.access.Unlock()
		go s.serveConn(conn, targetAddr)
	}
}

func (s *UDPRelayServer) Close() error {
	var err error
	s.access.Lock()
	...
	err = errors.Join(err, s.pc.Close())
	return err
}

func (s *UDPRelayServer) Addr() net.Addr {
	return s.pc.LocalAddr()
}

func (s *UDPRelayServer) serveConn(conn *quic.Conn, targetAddr string) {
	defer func() {
		s.access.Lock()
		delete(s.conns, conn)
		s.access.Unlock()
	...
	for {
		...
		if _, err = pc.WriteTo(data, targetUDPAddr); err != nil {
			return
		}
		select {
		case <-done:
